...
def user_register(request):
    if request.method == "POST":
        ...
        #一系列合法性判定
        if usernames == None or usernames == None or password1 == None or password2 == None or email == None or nickname == None or birthday == None or city == None:
            #信息不完整
            return render(request, 'msgboard/register.html', {'form':form, 'error_index':3})
        if not registerForm.is_valid() or password1 != password2:
            #注册失败  
            return render(request, 'msgboard/register.html', {'form':form, 'error_index':0})
        if len(User.objects.filter(username=usernames)):
            #用户名已存在
            return render(request, 'msgboard/register.html', {'form':form, 'error_index':1})
        #判定完毕
        
        myUser = MyUser()
        
        #send_mail('注册成功', '注册成功', "LinkAct@LinkAct.com", [email, 'xsx1996@163.com'], fail_silently=False)

        myUser.create_user(nickname, usernames, password1)
        
        return render(request, 'msgboard/success.html', {'username':usernames})
    else:
        form = RegisterForm()
